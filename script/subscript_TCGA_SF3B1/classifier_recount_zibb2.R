...
for (k in 1:2) {
  ...
  tD <- D %>% filter(Splicing_Key %in% junc) %>%
  ...
  for(n in 1:length(test_sample_name)) {
    ...
    tD2 <- tD %>% filter(Sample_Name %in% test_sample_name[n])
    sample_name <- unique(tD2 %>% .$Sample_Name)
    mutation_info <- unique(tD2 %>% .$Mutation_Info)
    cancer_type <- unique(tD2 %>% .$Cancer_Type)
    
    prob0 <- unlist(map(tD2 %>% transpose(), function(x) {get_prob_for_zibb(c(x$Param_1, x$Param_2, x$Param_3), x$Read_Count1 + x$Read_Count2, x$Read_Count1)}))
    prob1 <- unlist(map(tD2 %>% transpose(), function(x) {get_prob_for_zibb(c(x$Param_4, x$Param_5, x$Param_6), x$Read_Count1 + x$Read_Count2, x$Read_Count1)}))
