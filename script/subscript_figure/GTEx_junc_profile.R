...
junc_file <- "../output/recount2/GTEx/SRP012682.junction_coverage.filt.txt"
...
filtD2 <- filtD %>% filter(Sample_Name == "SRR1374647")
...
ggplot(filtD2, aes(x = Splicing_Key2, y = Count, fill = Splicing_Type)) + 
  ...
  labs(x = "Alternative 3'SS loci", y = "Count", fill = "") +
  scale_fill_manual(values = c("#f0027f", "#386cb0"))
  
ggsave("../output/recount2/GTEx/GTEx_junc_profile.tiff", width = 6, height = 6, units = "cm", dpi = 600)
